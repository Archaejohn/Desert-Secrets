...
writeFileSync("docs/superpowers/artifacts/ground-fills-review.html",
  `<!doctype html><meta charset=utf8><title>G1 ground fills</title><style>
   ...
   figcaption{font-size:11px;color:#999;text-align:center;margin-top:3px}</style>
   <h1>G1 ground fills — old tiled vs new world-position</h1>
   <p class="legend">Each ground at 96×96. Left = current recipe stamped (note the repeating 16px block). Right = the world-position fill (no repeat; adds macro tonal drift + per-material grain). Pixel-exact scaling.</p>
