...
- **Ramp monotonicity is load-bearing:** `cliffFace`/`quantize` re-index ramps by `ramp.indexOf(name)`; every ramp in `src/shared/palette.ts` and `tools/pipeline/src/cliffs/palette.ts` must stay monotonic in luminance after re-hexing. (inventory §2)
