...
Add a **ramp tile group** to the parametric generator, in two selectable ramp **materials** — `sandSlope` (smooth natural incline) and `stoneSteps` (carved stairs) — that auto-tile down the cliff's south face at flexible width and auto-scaling height, work in **both directions** (mirror-symmetric — descend left or right), and support **switchbacks** (via a flat `landing` turn-platform tile), reusing the existing `rock` cliff material for the cut's side walls and the terrain fills for the walkable surface.
...
4 width-columns × 4 height-rows = **16 tiles**:
...
- **Rows** (`RampRow`): `top` (the lip where the plateau opens onto the ramp), `run` (the slope surface, repeats ×height), `landing` (a **flat** turn platform — walkable, cliff-walled on its outer sides — where a switchback pivots/reverses), `bottom` (the foot where the ramp meets the ground).

Runtime placement (phase 2) picks the **column** from the ramp cell's left/right neighbors (same idea as the cliff south-edge variant selection) and the **row** from where the cell sits in the run. One set serves a 1-wide stair (`narrow`) through a 4-wide grand ramp (`leftEdge` + `middle`×2 + `rightEdge`).

### Both directions + switchbacks

- **Both directions.** The set is **mirror-symmetric**: the same tiles serve a ramp descending toward the left or the right (`PixelGrid.mirrorX()` — free). Directional pieces (`leftEdge`/`rightEdge`, and any directional slope lean) are generated as a mirror pair, so a scene can drop a ramp on either side of a plateau or angle it either way.
- **Switchbacks.** A tall descent is authored as `top → run(×k) → landing → run(×k) → … → bottom`, where each `landing` is a flat platform that lets the next flight **shift laterally / reverse** (the zigzag). The flights themselves stay roughly vertical (true-diagonal ramp tiles read poorly at 16px); the switchback shape comes from the landings' lateral offset. Phase 1b generates the pieces (`run`, `landing`, mirror-symmetric edges); **assembling a specific switchback layout is a placement/authoring concern (phase 2)**.
...
- **Names:** `rampSand_{col}_{row}` (sandSlope) and `rampSteps_{col}_{row}` (stoneSteps) — `col` ∈ {narrow, leftEdge, middle, rightEdge}, `row` ∈ {top, run, landing, bottom} — **32 new tiles** (16 per material). (`ramp` + material + col + row; anticipates a `SOLID`-adjacent registration but ramps are **walkable**, so a phase-2 note: register them **non-solid**, distinct from `cliffRock_*` which is solid.)
- **Sheet:** frame count 206 → **238**; re-pad to **240** (8 columns × 30 rows, 2 blank pad frames). Append-only after the existing groups — no existing frame index moves.
...
- **Structure tests** (`cliffs.test.ts`): `rampTiles("sandSlope", …)` and `("stoneSteps", …)` each return 16 tiles covering all 4 cols × 4 rows (incl. `landing`); palette-locked; deterministic; the total sheet name count = 238.
- **Determinism:** `mulberry32`/`h2` only; two builds byte-identical; sha256 re-pinned.
- **Visual review:** extend `render-cliff-review.mts` to render a demo scene with **ramps cut through the cliff** — a `sandSlope` straight ramp on one side, a `stoneSteps` ramp on the other, and **one switchback** (`top → run → landing → run → bottom`, mirrored) — to eyeball the incline read, the landing/turn, and the wall/plateau/ground connections before re-pinning.

## Non-goals (phase 2+)

- Runtime: the autotile assignment pass that detects ramp cells (from a map annotation) and lays the ramp columns/rows + flanking cliff, **assembling switchback layouts** (which flights/landings go where), `ZoneScene` wiring, and **collision/traversal** (letting the player actually walk the ramp between levels). Phase 1b generates the pieces; phase 2 arranges and wires them.
