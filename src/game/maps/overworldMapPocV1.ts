/**
 * **PROOF OF CONCEPT — SUPERSEDED, NOT WIRED INTO THE GAME.** This is a
 * frozen, byte-for-byte-renamed copy of `overworldMap.ts` exactly as it
 * shipped for docs/CONTRACTS.md "v22", kept only because the project owner
 * said "save it. we might come back to it" when rejecting this whole
 * approach in favor of the terrain-first generator ("v23") that now lives
 * in `overworldMap.ts`. Nothing in `src/` imports this file — not
 * `main.ts`, no scene, no test — it exists purely as dead-but-compiling
 * reference code. Every exported symbol is suffixed `_POC`/`Poc` (e.g.
 * `buildOverworldMapPoc`, `OVERWORLD_WIDTH_POC`) so it can never collide
 * with `overworldMap.ts`'s own exports if both are ever imported side by
 * side. See docs/CONTRACTS.md "v23" for why this approach was rejected:
 * in short, it built the map by carving mountain AROUND a hand-authored
 * winding path decided in advance, rather than generating organic terrain
 * first and placing landmarks into it afterward.
 *
 * Everything below this banner is UNCHANGED from the shipped v22 file
 * except identifier renames (`OVERWORLD_*` → `OVERWORLD_*_POC`,
 * `buildOverworldMap` → `buildOverworldMapPoc`) needed to keep this
 * self-contained. The original v22 module doc follows, unedited:
 *
 * ---
 *
...
export const OVERWORLD_WIDTH_POC = 64;
export const OVERWORLD_HEIGHT_POC = 64;

/** South-edge exit band → back to the oasis (the wash/spring stop). */
export const OVERWORLD_SOUTH_EXIT_POC = { x1: 31, y1: OVERWORLD_HEIGHT_POC - 1, x2: 33, y2: OVERWORLD_HEIGHT_POC - 1 } as const;
/** North-edge exit band → the mine entrance (the Cinnabar Mine stop). */
export const OVERWORLD_NORTH_EXIT_POC = { x1: 31, y1: 0, x2: 33, y2: 0 } as const;
/** Where the player appears arriving from the oasis (also the default spawn). */
export const OVERWORLD_SOUTH_SPAWN_POC = { x: 32, y: 61 } as const;
/** Where the player appears arriving back from the mine entrance. */
export const OVERWORLD_NORTH_SPAWN_POC = { x: 32, y: 2 } as const;
...
const PATH_WAYPOINTS: ReadonlyArray<readonly [number, number]> = [
  [OVERWORLD_HEIGHT_POC - 1, SPINE_CX],
  [58, SPINE_CX],
 ...
 * place before `buildOverworldMapPoc` returns. Every choice is a function of
...
export function buildOverworldMapPoc(): ZoneMap {
  const ground: string[][] = [];
  const decor: (string | null)[][] = [];

  for (let y = 0; y < OVERWORLD_HEIGHT_POC; y++) {
    ground.push([]);
    decor.push([]);
    for (let x = 0; x < OVERWORLD_WIDTH_POC; x++) {
  ...
  for (let y = SPINE_Y1; y <= SPINE_Y2; y++) {
    for (let x = spineLeft[y]; x <= spineRight[y]; x++) {
      if (x > 0 && x < OVERWORLD_WIDTH_POC - 1) decor[y][x] = MOUNTAIN_SENTINEL;
    }
  }
  ...
  for (let y = 1; y < OVERWORLD_HEIGHT_POC - 1; y++) {
    const cx = centerXForRow(y);
    for (let x = cx - 1; x <= cx + 1; x++) {
      if (x > 0 && x < OVERWORLD_WIDTH_POC - 1) decor[y][x] = null;
    }
  }
  ...
  const yTo = Math.min(OVERWORLD_HEIGHT_POC - 2, LAKE_CY + lakeSpanY);
  const xFrom = Math.max(1, Math.round(LAKE_CX - lakeSpanX));
  const xTo = Math.min(OVERWORLD_WIDTH_POC - 2, Math.round(LAKE_CX + lakeSpanX));
  ...
  // and off the road.
  for (let y = 1; y < OVERWORLD_HEIGHT_POC - 1; y++) {
    for (let x = 1; x < OVERWORLD_WIDTH_POC - 1; x++) {
  ...
  for (let x = 0; x < OVERWORLD_WIDTH_POC; x++) {
    ...
    decor[OVERWORLD_HEIGHT_POC - 1][x] = MOUNTAIN_SENTINEL;
  }
  for (let y = 0; y < OVERWORLD_HEIGHT_POC; y++) {
    decor[y][0] = MOUNTAIN_SENTINEL;
    decor[y][OVERWORLD_WIDTH_POC - 1] = MOUNTAIN_SENTINEL;
  }

  // Visible gates: the two stops, opened last so nothing above re-seals them.
  for (let x = OVERWORLD_SOUTH_EXIT_POC.x1; x <= OVERWORLD_SOUTH_EXIT_POC.x2; x++) {
    decor[OVERWORLD_HEIGHT_POC - 1][x] = null;
    ground[OVERWORLD_HEIGHT_POC - 1][x] = "sand2";
  }
  for (let x = OVERWORLD_NORTH_EXIT_POC.x1; x <= OVERWORLD_NORTH_EXIT_POC.x2; x++) {
    decor[OVERWORLD_NORTH_EXIT_POC.y1][x] = null;
    ground[OVERWORLD_NORTH_EXIT_POC.y1][x] = "sand2";
  }
